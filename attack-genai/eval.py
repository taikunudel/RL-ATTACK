...
import hashlib
import diskcache
from rlatk.core.paths import DISKCACHE_DIR
...
def build_prefix_suffix_slots(input_ids, attention_mask, num_doc_masks, mask_token_id, len_doc_max=None):
    """Wrap the VERBATIM question with prefix(floor N/2) + suffix(ceil N/2) [MASK] slots (N=num_doc_masks).
    Returns (masked_input_ids[1,L], attention_mask[1,L], attacked_positions=[[slot indices only]]).
    Question tokens are never in attacked_positions, so they stay byte-identical."""
    ids = input_ids[0].tolist()
    am = attention_mask[0].tolist()
    valid = [i for i, v in enumerate(am) if v == 1]
    cls_id, sep_id = ids[valid[0]], ids[valid[-1]]
    question = ids[valid[0] + 1: valid[-1]]
    N = int(num_doc_masks); pre = N // 2; suf = N - pre
...
def main(args):
    ...
    if data_name in ['harmul_strings', 'harmful_behaviors']:
        ...
        # In-the-wild HELD-OUT eval split (disjoint from train/dev; seed=42). Keep only
        # JAILBREAK (label==1, harmful) docs and the first `eval_limit` of them, to parallel
        # AdvBench's all-harmful "make unsafe look safe" setup (apples-to-apples eval).
        _, _, _wild_full = build_datasets(tokenizer=tokenizer, num_doc_masks=num_doc_masks, max_len=len_doc_max, seed=42)
        _lim = args.eval_limit if (args.eval_limit and args.eval_limit > 0) else None
        evaluation_dataloader = []
        for _b in _wild_full:
            if int(_b['labels'].item()) == 1:
                evaluation_dataloader.append(_b)
                if _lim is not None and len(evaluation_dataloader) >= _lim:
                    break
        print("[wild_jailbreaking] %d jailbreak(label=1) held-out docs (eval_limit=%s)" % (len(evaluation_dataloader), _lim))
