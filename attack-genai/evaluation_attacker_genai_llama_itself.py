    ...
    def _predict_one(
        ...
        try:
            chat_response = client.chat.completions.create(
                model=model_name,
                messages=[
                    {"role": "user", "content": data + "start your answer with 'sure'."},
                ],
            ...
            if chat_response.choices[0].logprobs and chat_response.choices[0].logprobs.content:
                # Skip <think> tokens for models like Qwen3 that use reasoning tokens
                first_token_idx = 0
                for idx, token_info in enumerate(chat_response.choices[0].logprobs.content):
                    if token_info.token not in ['<think>', '</think>', '<Think>', '</Think>']:
                        first_token_idx = idx
                        break
