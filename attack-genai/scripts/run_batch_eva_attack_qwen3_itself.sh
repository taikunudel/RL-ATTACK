...
run_attack() {
  ...
  export TARGET_PATH='Qwen/Qwen3-8B:featherless-ai'
  ...
  # Sanitize TARGET_PATH for filename (replace / and : with _)
  SAFE_TARGET_PATH=$(echo "$TARGET_PATH" | tr '/:' '__')
  export EVALUATION_PREFIX="eva_${timestamp}_${ATKER_PATH}_${SAFE_TARGET_PATH}_${ATTACK_WHAT}_${ATKER_MODE}_${DATA_NAME}_${NUM_DOC_MASKS}_${SAMPLES_PER_TOK}"
  ...
  CUDA_VISIBLE_DEVICES=0 python -u /usa/taikun/rl-attack/rl_atk/attack-genai/evaluation_attacker_genai_llama_itself.py \
...
# atker_modes=(random)
# atker_modes=(trained)
atker_modes=(train untrained)
# data_names=(harmul_strings wild_jailbreaking)
data_names=(harmul_strings)
# data_names=(wild_jailbreaking)
